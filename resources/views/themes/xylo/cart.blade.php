...
@foreach ($cart as $key => $item)
    @php
        ...
        $variant = $item['variant_model'] ?? null;
        $attributeValuesMap = $item['attribute_values_map'] ?? [];
                ...
                <div id="size-color-wrapper">
                    ...
                    @if (!empty($item['attributes']))
                        @foreach ($item['attributes'] as $attributeValueId)
                            @php
                                $attributeValue = $attributeValuesMap[$attributeValueId] ?? null;
                            @endphp
    ...
    @endforeach
                ...
                <div class="btn-group mt-4">
                    <a href="{{ route('xylo.home') }}" class="btn btn-light">{{ __('store.cart.continue_shopping') }}</a>
                    <button type="button" class="btn btn-primary update-cart">{{ __('store.cart.update_cart') }}</button>
                </div>
            </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        document.querySelectorAll('.remove-from-cart').forEach(button => {
            button.addEventListener('click', function() {
                ...
                .then(data => {
                    toastr.success(data.message || "Removed from cart!", "", {
                        closeButton: true,
                        progressBar: true,
