            ...
            <div class="tab-content mt-3">
                @foreach($languages as $language)
                    <div class="tab-pane fade show {{ old('active_tab', $languages->first()->code) === $language->code ? 'active' : '' }}" id="{{ $language->code }}">
                        ...
                        {{-- Subtitle (plain text) --}}
                        <label class="form-label mt-2">Subtitle ({{ $language->code }})</label>
                        <input type="text"
                               name="languages[{{ $language->code }}][subtitle]"
                               class="form-control @error('languages.' . $language->code . '.subtitle') is-invalid @enderror"
                               value="{{ old('languages.' . $language->code . '.subtitle') }}"
                               placeholder="Plain text subtitle for the hero section">
                        @error('languages.' . $language->code . '.subtitle')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror

                        {{-- CTA Text --}}
                        <label class="form-label mt-2">CTA Button Text ({{ $language->code }})</label>
                        <input type="text"
                               name="languages[{{ $language->code }}][cta_text]"
                               class="form-control @error('languages.' . $language->code . '.cta_text') is-invalid @enderror"
                               value="{{ old('languages.' . $language->code . '.cta_text') }}"
                               placeholder="e.g. View Products">
                        @error('languages.' . $language->code . '.cta_text')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror

                        {{-- CTA Link --}}
                        <label class="form-label mt-2">CTA Link ({{ $language->code }})</label>
                        <input type="text"
                               name="languages[{{ $language->code }}][cta_link]"
                               class="form-control @error('languages.' . $language->code . '.cta_link') is-invalid @enderror"
                               value="{{ old('languages.' . $language->code . '.cta_link') }}"
                               placeholder="e.g. /products or https://...">
                        @error('languages.' . $language->code . '.cta_link')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
