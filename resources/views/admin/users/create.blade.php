...
<div class="card mt-4">
    <div class="card-header card-header-bg text-white">
        <h6 class="d-flex align-items-center mb-0 dt-heading">{{ __('cms.users.title_create') }}</h6>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.users.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <small class="text-muted">{{ __('cms.users.password_hint') }}</small>
                @error('password')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            ...
            <button type="submit" class="btn btn-success">{{ __('cms.users.save') }}</button>
