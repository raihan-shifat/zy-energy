...
$(document).ready(function() {
    ...
    $('#menu-items-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "{{ route('admin.menus.item.getData', ['menu' => $menu->id]) }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}"
            },
        ...
        columns: [
                ...
                { 
                    ...
                    render: function(data, type, row) {  // <- Make sure this line is correct
                        ...
                        var deleteBtn = isManager ? '' : '<span class="border border-danger dt-trash rounded-3 d-inline-block" onclick="deleteMenuItem(' + row.id + ')"> <i class="bi bi-trash-fill text-danger"></i> </span>';
