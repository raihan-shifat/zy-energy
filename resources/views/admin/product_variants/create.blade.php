    ...
    <form action="{{ route('admin.product_variants.store') }}" method="POST">
        ...
        @foreach($languages as $language)
            <div class="form-group">
                ...
                <input type="text" name="translations[{{ $language->code }}][name]" id="translations_{{ $language->code }}_name" class="form-control" value="{{ old('translations.' . $language->code . '.name') }}" {{ $language->code === 'en' ? 'required' : '' }}>
