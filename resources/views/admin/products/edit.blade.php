        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="tab-content mt-3" id="languageTabContent">
                @foreach($activeLanguages as $language)
                    <div class="tab-pane fade show {{ $loop->first ? 'active' : '' }}" id="{{ $language->name }}" role="tabpanel">
                        ...
                        <input type="text" name="translations[{{ $language->code }}][name]" class="form-control @error("translations.{$language->code}.name") is-invalid @enderror" value="{{ old('translations.' . $language->code . '.name', $product->getTranslation('name', $language->code)) }}">
                        {{-- NOTE: no `required` here — 18 hidden language tabs with empty+required inputs
                             silently block form submission via HTML5 validation (tooltip can't render
                             inside display:none panes). Server-side rule enforces the EN name instead. --}}
                        @error("translations.{$language->code}.name")
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                        <label class="form-label">{{ __('cms.products.description') }} ({{ $language->code }})</label>
                        <textarea name="translations[{{ $language->code }}][description]" class="form-control ck-editor-multi-languages @error("translations.{$language->code}.description") is-invalid @enderror">{{ old('translations.' . $language->code . '.description', $product->getTranslation('description', $language->code)) }}</textarea>
            ...
            <div class="row mt-4">
                <div class="col-md-6">
                    ...
                    <input type="text" name="series" class="form-control" value="{{ old('series', $product->series) }}" placeholder="e.g. S-500">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Type</label>
                    <select name="type" id="product-type" class="form-control">
                        <option value="">-</option>
                        @foreach ($product->category->types ?? [] as $t)
                            <option value="{{ $t }}" {{ $product->type == $t ? 'selected' : '' }}>{{ $t }}</option>
                        @endforeach
                        @if ($product->type && !in_array($product->type, $product->category->types ?? []))
            ...
            <script>
                ...
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    if (categorySelect) {
                        categorySelect.addEventListener('change', function () { populateTypes(); });
                        populateTypes();
                    }
                ...
                <div class="variant-item mt-4" id="variant_{{ $variant->id }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mt-2">
                            ...
                            <input type="number" name="variants[{{ $index }}][stock]" class="form-control" value="{{ $variant->stock }}" min="0" max="2147483647" />
                        </div>
                        ...
                        @foreach($attributes as $attr)
                            <div class="col-md-6 mt-2">
                                ...
                                <select name="variants[{{ $index }}][attribute_values][{{ $attr->id }}]" class="form-control">
                                    <option value="">-- None --</option>
                                    @foreach($attr->values as $val)
                                        <option value="{{ $val->id }}" {{ ($variant->attribute_value_map[$attr->id] ?? null) == $val->id ? 'selected' : '' }}>{{ $val->value }}</option>
                                    @endforeach
                                </select>
                            </div>
                        @endforeach
           ...
           <button type="button" id="add-variant-btn"
            ...
            <div class="mt-4 text-start">
                <button type="submit" class="btn btn-primary" id="editSubmitBtn">{{ __('cms.products.save_product') }}</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    // Show validation errors for existing variants
    @foreach($product->variants as $idx => $variant)
        (function() {
            var i = {{ $idx }};
            if (validationErrors['variants.' + i + '.name']) {
                var el = document.querySelector('input[name="variants[' + i + '][name]"]');
                if (el) { el.classList.add('is-invalid'); }
            }
            if (validationErrors['variants.' + i + '.price']) {
                var el = document.querySelector('input[name="variants[' + i + '][price]"]');
                if (el) { el.classList.add('is-invalid'); }
            }
            if (validationErrors['variants.' + i + '.stock']) {
                var el = document.querySelector('input[name="variants[' + i + '][stock]"]');
                if (el) { el.classList.add('is-invalid'); }
            }
            if (validationErrors['variants.' + i + '.SKU']) {
                var el = document.querySelector('input[name="variants[' + i + '][SKU]"]');
                if (el) { el.classList.add('is-invalid'); }
            }
        })();
    @endforeach
...
<script type="text/template" id="variant-template">
    <div class="card p-3 mt-3 variant-item border rounded" id="variant_new___INDEX__">
        <h5>
        ...
        <div class="row">
            ...
            <div class="col-md-4 mt-2">
                <label>{{ __('cms.products.stock') }}</label>
                <input type="number" name="variants[__INDEX__][stock]" class="form-control" min="0" max="2147483647">
            </div>
            <div class="col-md-4 mt-2">
                <label>{{ __('cms.products.sku') }}</label>
...
<script>
    $('#editSubmitBtn').closest('form').on('submit', function () {
