...
use App\Http\Controllers\Store\EnquiryController;
use App\Http\Controllers\Store\NewsController as StoreNewsController;
...
/* B2B pages */
Route::get('/certification', [StoreCertificationController::class, 'index'])->name('certification.index');
Route::get('/news', [StoreNewsController::class, 'index'])->name('news.index');
Route::get('/news/{slug}', [StoreNewsController::class, 'show'])->name('news.show');
Route::post('/product/review/store', [ReviewController::class, 'store'])->name('review.store')
    ->middleware('auth.customer');
...
// Rate limit public enquiry endpoints (5 per minute per IP) to prevent spam
