...
class OrderService
{
    ...
    public function createOrderFromPaypal(array $paypalResult): Order
    {
        ...
        foreach ($cart as $key => $item) {
            // NOTE: array keys are composite cart keys ("cart_{pid}_{attrs}") — use the product_id inside the item
            OrderDetail::create([
                'order_id' => $order->id,
            ...
            \App\Models\ProductVariant::where('product_id', $item['product_id'])
                ...
                ->when(isset($item['variant_id']), fn ($q) => $q->where('id', $item['variant_id']))
